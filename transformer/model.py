...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        attention_scores = (query @ value.transpose(-2, -1)) / math.sqrt(d_k)
        if mask is not None:
    ...
    def forward(self, q, k, v, mask):
        ...
        key = key.view([query.shape[0], query.shape[1], self.h, self.d_k]).transpose(
            1, 2
        )
        value = value.view(
            [query.shape[0], query.shape[1], self.h, self.d_k]
...
class EncoderBlock(nn.Module):
    def __init__(
        ...
        self_feed_forward_block: FeedForwardBlock,
        dropout: float,
    ) -> None:
        super().__init__()
        self.self_attention_block = self_attention_block
        self.self_feed_forward_block = self_feed_forward_block
    ...
    def forward(self, x, src_mask):
        ...
        x = self.residual_connections[1](x, self.self_feed_forward_block)
        return x
        ...
        nn.ModuleList(
            [
                EncoderBlock(
                    ...
                    self_feed_forward_block=FeedForwardBlock(d_model, d_ff, dropout),
                    dropout=dropout,
                )
                for n in range(N)
            ]
